import { Body, Controller, Delete, Get, HttpCode, Param, ParseBoolPipe, Post, Put, Query, UseGuards } from '@nestjs/common';
import { AuthGuard } from '../auth/auth.guard';
import { CreateDeviceDto, CreateRtspDeviceDto, CreateWebcamDeviceDto, DeviceConnectionDto, PrepareWebcamPathDto, UpdateDeviceDto } from './devices.dto';
...
export class DevicesController {
  ...
  @Post('webcam') createWebcam(@Body() dto: CreateWebcamDeviceDto) { return this.devices.createWebcam(dto); }
  @Post('webcam/prepare-path') @HttpCode(200) prepareWebcamPath(@Body() dto: PrepareWebcamPathDto) { return this.devices.prepareWebcamPath(dto.streamPath); }
  @Post('rtsp') createRtsp(@Body() dto: CreateRtspDeviceDto) { return this.devices.createRtspDirect(dto); }
